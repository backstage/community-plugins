...
const useStyles = makeStyles(theme => ({
  ...
  scoreNumber:    { fontSize: '1.8rem', fontWeight: 800, lineHeight: 1, fontFamily: 'monospace' },
  scoreLabel:     { fontSize: '0.55rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', opacity: 0.7 },
  scoreClickHint: { fontSize: '0.5rem', color: '#94A3B8', fontFamily: 'monospace', marginTop: 2 },
  ...
  metaValue: { fontSize: '1.4rem', fontWeight: 800, fontFamily: 'monospace', lineHeight: 1.1 },
  metaLabel: { fontSize: '0.65rem', color: theme.palette.text.secondary, textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 4 },
  ...
  typeBadge: { fontFamily: 'monospace', fontSize: '0.7rem', fontWeight: 700, padding: '2px 8px', borderRadius: 4 },

  // ── Events table ──────────────────────────────────────────────────────────
  tableHeader:  { fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.06em', color: theme.palette.text.secondary },
  tableRow:     { '&:hover': { background: theme.palette.action.hover } },
  calculatedAt: { fontSize: '0.7rem', color: theme.palette.text.disabled, fontFamily: 'monospace' },
...
const TYPE_COLORS: Record<string, { bg: string; color: string; label: string }> = {
  ...
  'platform-ticket':  { bg: '#EDE9FE', color: '#7C3AED', label: 'Platform Ticket'  },
  'emergency-access': { bg: '#FFE4E6', color: '#BE123C', label: 'Emergency Access' },
 ...
 */
const TYPE_FIXES: Record<string, { title: string; fix: string; impact: string; effort: string }> = {
  'kubectl-exec': {
...
const SCORING_FORMULA_LINE1 = 'Score = min(100, round(weighted_total ÷ 50 × 100))';
const SCORING_FORMULA_LINE2 = 'weighted_total = Σ(points × 0.5^(age_days ÷ 7))';
const SCORING_FORMULA_LINE3 = 'Decay: today=100%  7d ago=50%  14d ago=25%  30d ago=~3%';
...
function SeverityIcon({ severity }: { severity: string }) {
  ...
  switch (severity) {
    case 'critical': return <ErrorIcon style={{ color, fontSize: 18 }} />;
    case 'high':     return <WarningIcon style={{ color, fontSize: 18 }} />;
    case 'medium':   return <InfoIcon style={{ color, fontSize: 18 }} />;
    default:         return <CheckCircleIcon style={{ color, fontSize: 18 }} />;
  }
}
...
async function generatePDF(
  ...
  const impactColor = (level: string): RGB =>
    level === 'High' ? [239, 68, 68] : level === 'Medium' ? [245, 158, 11] : [16, 185, 129];
  const effortColor = (level: string): RGB =>
    level === 'Low'  ? [16, 185, 129] : level === 'Medium' ? [245, 158, 11] : [239, 68, 68];
  ...
  const BADGE_X = 154, BADGE_Y = 10, BADGE_W = 40, BADGE_H = 38;
  doc.setFillColor(...scoreColor);
  doc.roundedRect(BADGE_X, BADGE_Y, BADGE_W, BADGE_H, 4, 4, 'F');
  doc.setTextColor(...PDF_COLORS.white);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(24);
  doc.text(String(frictionScore.score), BADGE_X + BADGE_W / 2, BADGE_Y + 20, { align: 'center' });
  doc.setFontSize(7);
  doc.text('FRICTION SCORE', BADGE_X + BADGE_W / 2, BADGE_Y + 27, { align: 'center' });
  doc.text(frictionScore.severity.toUpperCase(), BADGE_X + BADGE_W / 2, BADGE_Y + 33, { align: 'center' });
  ...
  const summaryMetrics = [
    ...
    { label: 'Overhead / Eng', value: `${frictionScore.overheadHoursPerEngineer}h`   },
  ...
  summaryMetrics.forEach((m, i) => {
    ...
    doc.setFont('helvetica', 'bold');   doc.setFontSize(13); doc.setTextColor(...PDF_COLORS.teal); doc.text(m.value, x, y + 11);
    doc.setFont('helvetica', 'normal'); doc.setFontSize(7);  doc.setTextColor(...PDF_COLORS.gray); doc.text(m.label.toUpperCase(), x, y + 18);
  });

  y += METRICS_H + 6;

  // ── WHY THIS SCORE ─────────────────────────────────────────────────────────
  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...PDF_COLORS.teal);
  doc.text('WHY THIS SCORE', MARGIN, y); y += 4;
  doc.setDrawColor(...PDF_COLORS.teal); doc.line(MARGIN, y, PAGE_W - MARGIN, y); y += 5;

  const sevInfo = SEVERITY_EXPLANATION[frictionScore.severity];
  if (sevInfo) {
    doc.setFont('helvetica', 'bold');   doc.setFontSize(9); doc.setTextColor(...PDF_COLORS.navy); doc.text(sevInfo.title, MARGIN, y); y += 5;
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(...PDF_COLORS.gray);
  ...
  doc.setFillColor(240, 253, 250); doc.rect(MARGIN, y, CONTENT_W, FORMULA_BOX_H, 'F');
  doc.setFont('helvetica', 'bold');   doc.setFontSize(7); doc.setTextColor(...PDF_COLORS.teal);
  doc.text('HOW THE SCORE IS CALCULATED', MARGIN + 5, y + 6);
  doc.setFont('helvetica', 'normal'); doc.setFontSize(7); doc.setTextColor(...PDF_COLORS.navy);
  ...
  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...PDF_COLORS.teal);
  doc.text('BREAKDOWN BY EVENT TYPE', MARGIN, y); y += 4;
  doc.setDrawColor(...PDF_COLORS.teal); doc.line(MARGIN, y, PAGE_W - MARGIN, y); y += 6;
  ...
  Object.entries(byType).sort((a, b) => b[1] - a[1]).forEach(([type, count]) => {
    ...
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(...PDF_COLORS.navy);
    doc.text(t.label, MARGIN, y);
    doc.setFillColor(226, 232, 240); doc.roundedRect(BAR_X, y - 4, BAR_MAX_W, 5, 2, 2, 'F');
    doc.setFillColor(...hexToRgb(t.color)); doc.roundedRect(BAR_X, y - 4, barW, 5, 2, 2, 'F');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(7); doc.setTextColor(...PDF_COLORS.gray);
    doc.text(`${count} (${pct}%)`, PAGE_W - MARGIN + 1, y, { align: 'right' });
    y += 8;
  });
  y += 4;
  ...
  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...PDF_COLORS.teal);
  doc.text('TOP FRICTION CONTRIBUTORS', MARGIN, y); y += 4;
  doc.setDrawColor(...PDF_COLORS.teal); doc.line(MARGIN, y, PAGE_W - MARGIN, y); y += 6;
  ...
  Object.entries(byActor)
    ...
    .forEach(([actor, count], i) => {
      ...
      doc.setFillColor(...rowBg); doc.rect(MARGIN, y - 4, CONTENT_W, 8, 'F');
      doc.setFont('helvetica', 'bold');   doc.setFontSize(8); doc.setTextColor(...PDF_COLORS.gray);  doc.text(`#${i + 1}`, MARGIN + 3, y);
      doc.setFont('helvetica', 'normal'); doc.setTextColor(...PDF_COLORS.navy);                      doc.text(actor, MARGIN + 13, y);
      doc.setFont('helvetica', 'bold');   doc.setTextColor(...PDF_COLORS.teal);
      doc.text(`${count} events (${pct}%)`, PAGE_W - MARGIN, y, { align: 'right' });
      y += 9;
    });
  y += 4;
  ...
  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...PDF_COLORS.teal);
  doc.text('HOW TO DECREASE THE SCORE', MARGIN, y); y += 4;
  doc.setDrawColor(...PDF_COLORS.teal); doc.line(MARGIN, y, PAGE_W - MARGIN, y); y += 5;

  const FIX_CARD_H = 35;

  Object.entries(byType).sort((a, b) => b[1] - a[1]).forEach(([type, count]) => {
    if (y > PAGE_BOTTOM - FIX_CARD_H) { doc.addPage(); y = 20; }
    ...
    doc.setFillColor(...hexToRgb(t.color)); doc.rect(MARGIN, y, 3, FIX_CARD_H, 'F');
    doc.setFillColor(...PDF_COLORS.light);  doc.rect(MARGIN + 3, y, CONTENT_W - 3, FIX_CARD_H, 'F');

    doc.setFont('helvetica', 'bold');   doc.setFontSize(9); doc.setTextColor(...PDF_COLORS.navy);
    doc.text(fix.title, MARGIN + 7, y + 6);
    doc.setFontSize(7); doc.setTextColor(...hexToRgb(t.color));
    doc.text(`${count} events`, PAGE_W - MARGIN - 1, y + 6, { align: 'right' });

    doc.setFont('helvetica', 'normal'); doc.setFontSize(7); doc.setTextColor(...PDF_COLORS.gray);
    const fixLines = doc.splitTextToSize(fix.fix, CONTENT_W - 20);
    doc.text(fixLines.slice(0, 2), MARGIN + 7, y + 12);

    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...ic); doc.text(`Impact: ${fix.impact}`, MARGIN + 7, y + 27);
    doc.setTextColor(...ec); doc.text(`Effort: ${fix.effort}`, MARGIN + 43, y + 27);
    y += FIX_CARD_H;
  });
  ...
  const projectedScore = Math.max(PROJECTED_SCORE_FLOOR, frictionScore.score - projectedRed);

  // Only show projected score in PDF when score > 0 and there is something to improve
  if (frictionScore.score > 0 && projectedRed > 0) {
    const PROJ_BOX_H = 28;
    doc.setFillColor(240, 253, 250); doc.rect(MARGIN, y, CONTENT_W, PROJ_BOX_H, 'F');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(9); doc.setTextColor(22, 101, 52);
    doc.text(`PROJECTED SCORE AFTER FIXING TOP ${PROJECTED_FIX_COUNT} EVENT TYPES`, MARGIN + 5, y + 7);
    doc.setFontSize(22); doc.setTextColor(...scoreColor);    doc.text(String(frictionScore.score), MARGIN + 5, y + 21);
    doc.setFontSize(10); doc.setTextColor(...PDF_COLORS.gray); doc.text('to', MARGIN + 20, y + 21);
    doc.setFontSize(22); doc.setTextColor(16, 185, 129);     doc.text(`~${projectedScore}`, MARGIN + 24, y + 21);
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(22, 101, 52);
    doc.text(`Fixing top ${PROJECTED_FIX_COUNT} event types could reduce score by ~${projectedRed} points`, MARGIN + 45, y + 18);
    y += PROJ_BOX_H + 4;
  }
  ...
  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...PDF_COLORS.teal);
  doc.text('ALL FRICTION EVENTS', MARGIN, y); y += 4;
  doc.setDrawColor(...PDF_COLORS.teal); doc.line(MARGIN, y, PAGE_W - MARGIN, y); y += 5;

  // Table column x-positions
  const COL = { type: MARGIN + 3, actor: 55, workflow: 95, desc: 130, time: PAGE_W - MARGIN };

  doc.setFillColor(...PDF_COLORS.navy); doc.rect(MARGIN, y - 4, CONTENT_W, 8, 'F');
  ...
  recentEvents.forEach((event, i) => {
    ...
    doc.setFillColor(...rowBg); doc.rect(MARGIN, y - 4, CONTENT_W, 8, 'F');
    doc.setFont('helvetica', 'bold');   doc.setFontSize(7); doc.setTextColor(...hexToRgb(t.color));
    doc.text(t.label, COL.type, y);
    doc.setFont('helvetica', 'normal'); doc.setTextColor(...PDF_COLORS.navy);
    doc.text(String(event.actor       ?? '').split('@')[0].slice(0, 12), COL.actor,    y);
    doc.text(String(event.workflow    ?? '—').slice(0, 10),               COL.workflow, y);
    doc.text(String(event.description ?? '—').slice(0, 30),               COL.desc,     y);
    doc.setTextColor(...PDF_COLORS.gray);
    doc.text(relativeTime(event.timestamp), COL.time - 2, y, { align: 'right' });
    y += 8;
  });
  ...
  for (let pg = 1; pg <= pageCount; pg++) {
    doc.setPage(pg);
    doc.setFillColor(...PDF_COLORS.navy); doc.rect(0, FOOTER_Y, PAGE_W, 10, 'F');
    doc.setFont('helvetica', 'normal'); doc.setFontSize(7); doc.setTextColor(148, 163, 184);
    doc.text('Healert · Friction Intelligence Platform · healert.io', MARGIN, FOOTER_Y + 6);
    doc.text(`Page ${pg} of ${pageCount}`, PAGE_W - MARGIN, FOOTER_Y + 6, { align: 'right' });
  }

  // ── SAVE ───────────────────────────────────────────────────────────────────
  const filename = `healert-report-${entityRef.replace(/[:/]/g, '-')}-${new Date().toISOString().slice(0, 10)}.pdf`;
  doc.save(filename);
}
...
function ScoreDetailsModal({
  ...
  const topTypes  = Object.entries(byType).sort((a, b)  => b[1] - a[1]);
  const topActors = Object.entries(byActor).sort((a, b) => b[1] - a[1]).slice(0, MODAL_MAX_CONTRIBUTORS);
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      await generatePDF(frictionScore, recentEvents, byType, byActor, entityRef);
    } finally {
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <div className={classes.dialogHeader}>
        <Box display="flex" alignItems="center" style={{ gap: 16 }}>
          <div className={classes.modalScoreCircle} style={{ borderColor: color }}>
            <span style={{ fontSize: '1.6rem', fontWeight: 800, color, fontFamily: 'monospace', lineHeight: 1 }}>
              {frictionScore.score}
            </span>
            <span style={{ fontSize: '0.5rem', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              /100
            </span>
          </div>
          <div>
            <Typography style={{ fontWeight: 800, fontSize: '1.1rem', color: '#fff' }}>
              Friction Score Analysis Card
            </Typography>
            <Typography style={{ fontSize: '0.75rem', color: '#94A3B8', fontFamily: 'monospace' }}>
              {frictionScore.bypassCount} bypass events · {frictionScore.overheadHoursPerEngineer}h overhead/engineer/month
            </Typography>
          </div>
        </Box>
        <Box display="flex" alignItems="center" style={{ gap: 8 }}>
          ...
          <IconButton size="small" onClick={onClose} style={{ color: '#64748B' }}>
            <CloseIcon />
          </IconButton>
        </Box>
      </div>
      ...
      <DialogContent style={{ padding: 24 }}>
        <Box display="flex" flexDirection="column" style={{ gap: 24 }}>
          ...
          <div>
            <Typography className={classes.sectionTitle}>Why this score?</Typography>

            {/* Severity explanation */}
            <Box style={{ background: `${color}10`, border: `1.5px solid ${color}30`, borderRadius: 8, padding: '14px 16px' }}>
              <Typography style={{ fontWeight: 800, fontSize: '0.9rem', color, marginBottom: 6 }}>
                {sevInfo?.title}
              </Typography>
              <Typography style={{ fontSize: '0.82rem', color: '#475569', lineHeight: 1.7 }}>
                {sevInfo?.desc}
              </Typography>
            </Box>

            {/* Scoring formula — derived from EVENT_POINTS, never hardcoded */}
            <Box style={{ marginTop: 12, background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 8, padding: '12px 16px' }}>
              <Typography style={{ fontWeight: 700, fontSize: '0.75rem', color: '#64748B', fontFamily: 'monospace', textTransform: 'uppercase', marginBottom: 8 }}>
                How the score is calculated
              </Typography>
              <Box style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
                {Object.entries(EVENT_POINTS)
                  .sort(([, a], [, b]) => b - a)
                  .map(([type, pts]) => {
                    const t = getTypeStyle(type);
                    return (
                      <span key={type} style={{ background: t.bg, color: t.color, fontFamily: 'monospace', fontSize: '0.68rem', fontWeight: 700, padding: '3px 10px', borderRadius: 4 }}>
                        {t.label}: {pts}pts
                      </span>
                    );
                  })}
              </Box>
              <Typography style={{ fontSize: '0.70rem', color: '#475569', fontFamily: 'monospace', marginBottom: 4 }}>
                Score = min(100, round(weighted_total ÷ 50 × 100))
              </Typography>
              <Typography style={{ fontSize: '0.70rem', color: '#475569', fontFamily: 'monospace', marginBottom: 8 }}>
                weighted_total = Σ (points × 0.5^(age_days ÷ 7))
              </Typography>
              <Box style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {[
                  { label: 'today',   weight: '100%', bg: '#DCFCE7', color: '#16A34A' },
                  { label: '7d ago',  weight: '50%',  bg: '#FEF9C3', color: '#CA8A04' },
                  { label: '14d ago', weight: '25%',  bg: '#FEF3C7', color: '#D97706' },
                  { label: '30d ago', weight: '~3%',  bg: '#FEE2E2', color: '#DC2626' },
                ].map(({ label, weight, bg, color: c }) => (
                  <span key={label} style={{ background: bg, color: c, fontFamily: 'monospace', fontSize: '0.65rem', fontWeight: 700, padding: '2px 8px', borderRadius: 4 }}>
                    {label} = {weight}
                  </span>
                ))}
              </Box>
              <Typography style={{ fontSize: '0.65rem', color: '#94A3B8', fontFamily: 'monospace', marginTop: 8 }}>
          ...
          <div>
            <Typography className={classes.sectionTitle}>Top Friction Contributors</Typography>
            {topActors.map(([actor, count], i) => {
              const pct = Math.round((count / Math.max(recentEvents.length, 1)) * 100);
              return (
                <div key={actor} className={classes.contributorRow}>
                  <Box display="flex" alignItems="center" style={{ gap: 10 }}>
                    <span style={{ fontFamily: 'monospace', fontSize: '0.7rem', color: '#94A3B8', fontWeight: 700, minWidth: 20 }}>
                      #{i + 1}
                    </span>
                    <div>
                      <Typography style={{ fontWeight: 700, fontSize: '0.82rem', color: '#0F172A' }}>{actor}</Typography>
                      <Typography style={{ fontSize: '0.68rem', color: '#94A3B8', fontFamily: 'monospace' }}>{count} bypass events</Typography>
                    </div>
                  </Box>
                  <Box display="flex" alignItems="center" style={{ gap: 8 }}>
                    ...
                    <span style={{ fontFamily: 'monospace', fontSize: '0.7rem', color, fontWeight: 700 }}>{pct}%</span>
                  </Box>
                </div>
              );
            })}
          ...
          <div>
            <Box display="flex" alignItems="center" style={{ gap: 8, marginBottom: 12 }}>
              ...
              <Typography className={classes.sectionTitle}>How to Decrease the Score</Typography>
            </Box>
            {topTypes.map(([type, count]) => {
              const fix         = getTypeFix(type);
              const t           = getTypeStyle(type);
              const impactColor = fix.impact === 'High' ? '#EF4444' : fix.impact === 'Medium' ? '#F59E0B' : '#10B981';
              const effortColor = fix.effort === 'Low'  ? '#10B981' : fix.effort === 'Medium' ? '#F59E0B' : '#EF4444';
              return (
                <div key={type} className={classes.fixCard} style={{ borderLeftColor: t.color }}>
                  <Box display="flex" alignItems="flex-start" justifyContent="space-between">
                    <Typography style={{ fontWeight: 700, fontSize: '0.88rem', color: '#0F172A', marginBottom: 4 }}>
                      {fix.title}
                    </Typography>
                    <span style={{ background: t.bg, color: t.color, fontFamily: 'monospace', fontSize: '0.65rem', fontWeight: 700, padding: '2px 8px', borderRadius: 4, flexShrink: 0, marginLeft: 8 }}>
                      {count} events
                    </span>
                  </Box>
                  <Typography style={{ fontSize: '0.78rem', color: '#475569', lineHeight: 1.7 }}>
                    {fix.fix}
                  </Typography>
                  <Box display="flex" style={{ gap: 6, marginTop: 8 }}>
                    <span className={classes.fixBadge} style={{ background: `${impactColor}15`, color: impactColor }}>Impact: {fix.impact}</span>
                    <span className={classes.fixBadge} style={{ background: `${effortColor}15`, color: effortColor }}>Effort: {fix.effort}</span>
                  </Box>
                </div>
              );
            })}
          ...
          <div>
            ...
            <Box style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 8, padding: '12px 16px' }}>
              <Box display="flex" alignItems="center" style={{ gap: 16 }}>
                <div>
                  <Typography style={{ fontSize: '0.68rem', color: '#64748B', fontFamily: 'monospace' }}>Current</Typography>
                  <Typography style={{ fontSize: '2rem', fontWeight: 800, fontFamily: 'monospace', color }}>{frictionScore.score}</Typography>
                </div>
                <TrendingDownIcon style={{ color: '#10B981', fontSize: 28 }} />
                <div>
                  <Typography style={{ fontSize: '0.68rem', color: '#64748B', fontFamily: 'monospace' }}>After fixes</Typography>
                  <Typography style={{ fontSize: '2rem', fontWeight: 800, fontFamily: 'monospace', color: '#10B981' }}>~{projectedScore}</Typography>
                </div>
                <div style={{ flex: 1 }}>
                  <Typography style={{ fontSize: '0.78rem', color: '#166534', lineHeight: 1.6 }}>
                    Fixing <strong>{topTypes.slice(0, PROJECTED_FIX_COUNT).map(([t]) => getTypeStyle(t).label).join(' + ')}</strong> could
                    reduce your score by <strong style={{ color: '#10B981' }}>{projectedRed} points</strong>.
                  </Typography>
                </div>
              </Box>
            </Box>
...
export function FrictionScoreCard() {
  ...
  // ── Loading state ──────────────────────────────────────────────────────────
  if (loading) {
    return <InfoCard title="Friction Analysis Board"><Progress /></InfoCard>;
  }
      ...
      <InfoCard title="Friction Analysis Board">
        <Box style={{ padding: '24px 16px' }}>
          ...
          >
            ...
            <Box>
              {/* Title */}
              <Typography style={{ fontWeight: 700, fontSize: '0.95rem', marginBottom: 6,
                color: isUnreachable ? '#92400E' : isAuthError ? '#991B1B' : '#92400E' }}>
                {isUnreachable
              ...
              {/* Description */}
              <Typography style={{ fontSize: '0.85rem', marginBottom: 12,
                color: isUnreachable ? '#78350F' : isAuthError ? '#7F1D1D' : '#78350F' }}>
                {isUnreachable
              ...
              {/* Fix instructions */}
              <Box style={{ background: isUnreachable ? '#FEF3C7' : isAuthError ? '#FEE2E2' : '#FEF3C7',
                border: `1px solid ${isUnreachable ? '#FCD34D' : isAuthError ? '#FCA5A5' : '#FCD34D'}`,
    ...
    return (
      <InfoCard title="Friction Analysis Board">
        <Box style={{ padding: '40px 16px', textAlign: 'center' }}>
          ...
          <Typography style={{ fontWeight: 700, fontSize: '1rem', color: '#065F46', marginTop: 12 }}>
            No friction events recorded
          </Typography>
          <Typography style={{ fontSize: '0.85rem', color: '#6B7280', marginTop: 6 }}>
            This service has a clean record. No platform bypass events detected.
          </Typography>
          <Typography style={{ fontSize: '0.78rem', color: '#9CA3AF', marginTop: 8, fontFamily: 'monospace' }}>
    ...
    return (
      <InfoCard title="Friction Analysis Board">
        <Box style={{ padding: '24px 16px' }}>
          ...
          >
            <span style={{ fontSize: 26, lineHeight: 1, marginTop: 2 }}>⚠️</span>
            <Box>
              <Typography style={{ fontWeight: 700, fontSize: '0.95rem', marginBottom: 6, color: '#92400E' }}>
                Healert agent is not running
              </Typography>
              <Typography style={{ fontSize: '0.85rem', marginBottom: 12, color: '#78350F' }}>
                The backend is reachable but the Go agent has not connected yet.
                No Kubernetes audit log events are being detected.
              </Typography>
              <Box style={{ background: '#FEF3C7', border: '1px solid #FCD34D',
                borderRadius: 6, padding: '8px 14px',
                fontFamily: 'monospace', fontSize: '0.8rem', color: '#78350F' }}>
                ./healert.sh start
              </Box>
            </Box>
          </Box>
  ...
  const color    = SEVERITY_COLORS[frictionScore.severity] ?? '#64748B';
  const sevLabel = frictionScore.severity.charAt(0).toUpperCase() + frictionScore.severity.slice(1);
  ...
  const topFrictionType = Object.entries(byType).sort((a, b) => b[1] - a[1])[0]?.[0];
  ...
  const paginated = filteredEvents.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  const hasFilter = !!(activeFilter || actorFilter || workflowFilter || sevFilter || overheadFilter);
  ...
  const cardAction = (
    <Tooltip title={sevFilter ? 'Click to clear filter' : 'Click to show highest friction events'} arrow>
      <Chip
        size="small"
      ...
      <InfoCard title="Friction Analysis Board" action={cardAction}>
        <Box display="flex" flexDirection="column" style={{ gap: 24 }}>
          ...
          <Grid container spacing={2} alignItems="center">
            ...
            {/* Score circle */}
            <Grid item>
              <Tooltip title="Click for score details and improvement recommendations" arrow>
                <Box
                  className={classes.scoreCircle}
                  style={{ borderColor: color }}
                  onClick={() => setModalOpen(true)}
                >
                  <span className={classes.scoreNumber} style={{ color }}>{frictionScore.score}</span>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} sm={3}>
                  <div className={classes.metaCard}>
                    <div className={classes.metaValue} style={{ color }}>{frictionScore.bypassCount}</div>
                ...
                <Grid item xs={6} sm={3}>
                  <Tooltip
                    title={overheadFilter
                      ? 'Click to clear overhead filter'
                      : `Click to show high-overhead events (${OVERHEAD_TYPES.map(getTypeLabel).join(', ')})`}
                    arrow
                  >
                    <div
                      ...
                      style={{
                        ...
                        border:     overheadFilter ? '1.5px solid #F97316' : undefined,
                        transition: 'all 0.15s',
                      }}
                    >
                      <div className={classes.metaValue} style={{ color: overheadFilter ? '#F97316' : SEVERITY_COLORS.medium }}>
                        {frictionScore.overheadHoursPerEngineer}h
                      </div>
                      <div className={classes.metaLabel}>
                        Overhead / Eng <span style={{ fontSize: '0.55rem' }}>▼ click</span>
                      </div>
                    </div>
                  </Tooltip>
                </Grid>
                ...
                <Grid item xs={6} sm={3}>
                  ...
                  >
                    <div
                      className={classes.metaCard}
                      onClick={() => frictionScore.topFrictionWorkflow && handleWorkflowClick(frictionScore.topFrictionWorkflow)}
                      style={{
                        cursor:     'pointer',
                        background: workflowFilter ? '#F0FDFA' : undefined,
                        border:     workflowFilter ? '1.5px solid #0D9488' : undefined,
                        transition: 'all 0.15s',
                      }}
                    >
                      <div className={classes.metaValue} style={{ color: '#0D9488', fontSize: '0.9rem' }}>
                        {frictionScore.topFrictionWorkflow ?? '—'}
                      </div>
                      <div className={classes.metaLabel}>
                        Top Workflow <span style={{ fontSize: '0.55rem' }}>▼ click</span>
                      </div>
                    </div>
                  </Tooltip>
                </Grid>
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        background: actorFilter === topActor?.[0] ? '#F3E8FF' : undefined,
                        border:     actorFilter === topActor?.[0] ? '1.5px solid #7C3AED' : undefined,
                        transition: 'all 0.15s',
                      }}
                    >
                      <div className={classes.metaValue} style={{ color: '#7C3AED', fontSize: '0.85rem' }}>
                        {topActor ? topActor[0].split('@')[0] : '—'}
                      </div>
                      <div className={classes.metaLabel}>
                        Top Actor {topActor && <span style={{ fontSize: '0.55rem' }}>▼ click</span>}
                      </div>
                    </div>
                  </Tooltip>
                </Grid>
          ...
          <div>
            <Typography className={classes.sectionTitle}>
              Breakdown by Type
              {hasFilter && (
                <span className={classes.clearFilter} onClick={clearAllFilters} role="button">
                  ✕ clear all filters
                </span>
              )}
            </Typography>
            ...
            {hasFilter && (
              <Typography style={{ fontSize: '0.68rem', color: '#64748B', fontFamily: 'monospace', marginTop: 8 }}>
                Showing {filteredEvents.length} of {recentEvents.length} events
                {activeFilter   && <span>&nbsp;·&nbsp;<span style={{ color: getTypeStyle(activeFilter).color, fontWeight: 700 }}>{getTypeStyle(activeFilter).label}</span> only</span>}
                {actorFilter    && <span>&nbsp;·&nbsp;<span style={{ color: '#7C3AED', fontWeight: 700 }}>{actorFilter.split('@')[0]}</span> only</span>}
                {workflowFilter && <span>&nbsp;·&nbsp;<span style={{ color: '#0D9488', fontWeight: 700 }}>{workflowFilter}</span> workflow only</span>}
                {sevFilter      && <span>&nbsp;·&nbsp;<span style={{ color, fontWeight: 700 }}>Highest friction events only</span></span>}
                {overheadFilter && <span>&nbsp;·&nbsp;<span style={{ color: '#F97316', fontWeight: 700 }}>High overhead events only</span></span>}
              </Typography>
            )}
          </div>
          ...
          <div>
            ...
            <Typography className={classes.sectionTitle}>
              {overheadFilter  ? `High Overhead Events (${filteredEvents.length})`
               : sevFilter     ? `Highest Friction Events (${filteredEvents.length})`
               : workflowFilter? `${workflowFilter} Workflow Events (${filteredEvents.length})`
               : actorFilter   ? `${actorFilter.split('@')[0]} Events (${filteredEvents.length})`
               : activeFilter  ? `${getTypeStyle(activeFilter).label} Events (${filteredEvents.length})`
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell className={classes.tableHeader}>Workflow</TableCell>
                  ...
                  {paginated.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} align="center">
                        <Typography style={{ fontSize: '0.8rem', color: '#94A3B8', padding: '16px 0', fontFamily: 'monospace' }}>
                    ...
                    paginated.map((event: any, i: number) => {
                      ...
                      return (
                        <TableRow key={i} className={classes.tableRow}>
                          <TableCell>
                            <span className={classes.typeBadge} style={{ background: t.bg, color: t.color }}>
                              {t.label}
                            </span>
                          </TableCell>
                          <TableCell>
                            <Typography variant="body2" style={{ fontFamily: 'monospace', fontSize: '0.78rem' }}>
                              {event.actor}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Typography variant="body2" style={{ fontSize: '0.78rem' }}>
                              {event.workflow}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Typography variant="body2" style={{ fontSize: '0.78rem', color: '#475569' }}>
                              {event.description}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Typography
                              variant="body2"
                              style={{ fontFamily: 'monospace', fontSize: '0.72rem', color: '#94A3B8', whiteSpace: 'nowrap' }}
          ...
          <Box display="flex" alignItems="center" justifyContent="space-between" flexWrap="wrap" style={{ gap: 8 }}>
            <Box display="flex" style={{ gap: 6, flexWrap: 'wrap' }}>
              <Chip size="small" label="✓ K8s Audit Log" style={{ background: '#0D948820', color: '#0D9488', fontSize: '0.7rem', fontWeight: 700 }} />
              <Chip size="small" label="○ GitHub"     style={{ background: '#F1F5F9', color: '#94A3B8', fontSize: '0.7rem' }} />
              <Chip size="small" label="○ ArgoCD"     style={{ background: '#F1F5F9', color: '#94A3B8', fontSize: '0.7rem' }} />
              <Chip size="small" label="○ Jira"       style={{ background: '#F1F5F9', color: '#94A3B8', fontSize: '0.7rem' }} />
              <Chip size="small" label="○ PagerDuty"  style={{ background: '#F1F5F9', color: '#94A3B8', fontSize: '0.7rem' }} />
              <Chip size="small" label="○ GitLab"     style={{ background: '#F1F5F9', color: '#94A3B8', fontSize: '0.7rem' }} />
            </Box>
            <Box display="flex" flexDirection="column" alignItems="flex-end">
              <Typography className={classes.calculatedAt}>Updated: {relativeTime(frictionScore.calculatedAt)}</Typography>
              <Typography style={{ fontSize: '0.62rem', color: '#CBD5E1', fontFamily: 'monospace', marginTop: 2 }}>
