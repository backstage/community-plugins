...
const useStyles = makeStyles(theme => ({
  ...
  heatmapGrid: { display: 'inline-grid', gap: 6, minWidth: 300, alignItems: 'end' },
  ...
  legendLabel: {
    ...
    color: '#64748B', verticalAlign: 'middle',
  },

  // Drill-down panel
  drillPanel: {
    background: '#F8FAFC', border: '1px solid #E2E8F0',
    borderRadius: 10, padding: theme.spacing(2.5),
    marginTop: theme.spacing(2), position: 'relative',
  },
  drillHeader: {
    display: 'flex', alignItems: 'flex-start',
    justifyContent: 'space-between', marginBottom: theme.spacing(2),
  },
  eventRow: {
  ...
  sectionLabel: {
    ...
    color: '#64748B', fontFamily: 'monospace', fontSize: '0.62rem', marginBottom: 8,
  },
}));
    ...
    <div className={classes.drillPanel}>
      ...
      <div className={classes.drillHeader}>
        <div>
          <Box display="flex" alignItems="center" style={{ gap: 10, marginBottom: 4 }}>
            <span style={{
              background: pal.bg, color: pal.text, border: `1.5px solid ${pal.border}`,
              fontFamily: 'monospace', fontSize: '0.7rem', fontWeight: 700,
              padding: '3px 10px', borderRadius: 5,
            }}>
              {cell.severity.toUpperCase()}
            </span>
            <Typography style={{ fontWeight: 800, fontSize: '1rem', color: '#0F172A' }}>
              {cell.team} &times; {getTypeLabel(cell.workflow)}
            </Typography>
          </Box>
          <Typography style={{ fontSize: '0.72rem', color: '#64748B', fontFamily: 'monospace' }}>
            {cell.deviationCount} bypass event{cell.deviationCount !== 1 ? 's' : ''} &nbsp;·&nbsp; Friction score: {cell.frictionScore}
          </Typography>
        </div>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={4}>
          <Typography className={classes.sectionLabel}>Friction Score</Typography>
          <Typography style={{
            fontFamily: 'monospace', fontSize: '2.6rem',
            fontWeight: 800, color: pal.text, lineHeight: 1,
          }}>
            {cell.frictionScore}
          </Typography>
          <Typography style={{ fontSize: '0.62rem', color: '#94A3B8', fontFamily: 'monospace', marginTop: 4 }}>
            / 100
          </Typography>
          <div style={{ marginTop: 8, width: '100%', height: 6, background: '#E2E8F0', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ width: `${cell.frictionScore}%`, height: '100%', background: pal.text, borderRadius: 3 }} />
          </div>
        </Grid>
        ...
        <Grid item xs={12} sm={4}>
          ...
          {cell.topActors.length === 0 ? (
            ...
            cell.topActors.map((actor, i) => (
              <Box key={actor} display="flex" alignItems="center" style={{ gap: 8, marginBottom: 6 }}>
                <span style={{ fontFamily: 'monospace', fontSize: '0.62rem', color: '#94A3B8', minWidth: 20 }}>
                  #{i + 1}
                </span>
                <Typography style={{ fontSize: '0.78rem', fontFamily: 'monospace', color: '#0F172A' }}>
                  {actor.split('@')[0]}
                </Typography>
              </Box>
            ))
        ...
        <Grid item xs={12} sm={4}>
          ...
          <Chip
            ...
            style={{ background: '#FEE2E2', color: '#DC2626', fontWeight: 700, fontSize: '0.72rem' }}
          />
          <Typography style={{ fontSize: '0.72rem', color: '#64748B', marginTop: 8, lineHeight: 1.6 }}>
            {cell.deviationCount} event{cell.deviationCount !== 1 ? 's' : ''} in the <strong>{cell.team}</strong> workflow
          </Typography>
        </Grid>
      </Grid>
      ...
      {cell.recentEvents.length > 0 && (
        <>
          ...
          <Typography className={classes.sectionLabel}>Recent Events</Typography>
          {cell.recentEvents.map((evt, i) => (
            <div key={i} className={classes.eventRow}>
              <Box display="flex" alignItems="center" style={{ gap: 10 }}>
                <span className={classes.typeBadge}>{getTypeLabel(evt.type)}</span>
                <div>
                  <Typography style={{ fontWeight: 700, fontSize: '0.75rem', color: '#0F172A' }}>
                    {evt.actor.split('@')[0]}
                  </Typography>
              ...
              <Typography style={{
                ...
                color: '#94A3B8', whiteSpace: 'nowrap', marginLeft: 8,
              }}>
...
export function FrictionHeatmap() {
  ...
  // ── Loading state ──────────────────────────────────────────────────────────
  if (loading) {
    return <InfoCard title="Friction Heatmap"><Progress /></InfoCard>;
  }
    ...
    return (
      <InfoCard title="Friction Heatmap">
        <Typography style={{
          ...
          No friction events yet for <strong>{entityRef}</strong>.
          Send events to the backend to populate the heatmap.
        </Typography>
      </InfoCard>
    );
  }
  ...
  const cardAction = (
    <Typography style={{ fontSize: '0.65rem', color: '#94A3B8', fontFamily: 'monospace' }}>
      {entityRef} · Updated {relativeTime(updatedAt)}
    </Typography>
  );

  return (
    <InfoCard title="Friction Heatmap — Workflow × Event Type" action={cardAction}>
      <Box display="flex" flexDirection="column" style={{ gap: 20 }}>

        {/* ── Summary stats row ── */}
        <Grid container spacing={2}>
          <Grid item xs={6} sm={3}>
            <Box style={{ background: '#FDF2F8', border: '1px solid #F0ABFC', borderRadius: 8, padding: '10px 14px', textAlign: 'center' }}>
              <Typography style={{ fontFamily: 'monospace', fontSize: '1.6rem', fontWeight: 800, color: '#86198F', lineHeight: 1 }}>
                {criticalN}
              </Typography>
              <Typography style={{ fontSize: '0.62rem', color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 3 }}>
                Critical Cells
              </Typography>
            </Box>
          </Grid>
          <Grid item xs={6} sm={3}>
            <Box style={{ background: '#FEE2E2', border: '1px solid #FCA5A5', borderRadius: 8, padding: '10px 14px', textAlign: 'center' }}>
              <Typography style={{ fontFamily: 'monospace', fontSize: '1.6rem', fontWeight: 800, color: '#DC2626', lineHeight: 1 }}>
                {highN}
              </Typography>
              <Typography style={{ fontSize: '0.62rem', color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 3 }}>
                High Friction
          ...
          <Grid item xs={6} sm={3}>
            <Box style={{ background: '#F0FDFA', border: '1px solid #99F6E4', borderRadius: 8, padding: '10px 14px', textAlign: 'center' }}>
              <Typography style={{ fontFamily: 'monospace', fontSize: '1.6rem', fontWeight: 800, color: '#0D9488', lineHeight: 1 }}>
                {avgScore}
              </Typography>
              <Typography style={{ fontSize: '0.62rem', color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 3 }}>
                Avg Score
          ...
          <Grid item xs={6} sm={3}>
            <Box style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 8, padding: '10px 14px', textAlign: 'center' }}>
              <Typography style={{ fontFamily: 'monospace', fontSize: '1.6rem', fontWeight: 800, color: '#475569', lineHeight: 1 }}>
                    ...
                    <Tooltip
                      ...
                      title={
                        <div>
                          ...
                          <div>{cell.deviationCount} event{cell.deviationCount !== 1 ? 's' : ''}</div>
                          {cell.topActors[0] && (
                            <div>Top actor: {cell.topActors[0].split('@')[0]}</div>
                          )}
                        </div>
                      }
                      arrow
                    >
                      <div
                        ...
                        style={{
                          ...
                          boxShadow:   isSelected ? `0 0 0 3px ${pal.text}40` : undefined,
                        }}
                      >
                        ...
                        {cell.severity !== 'none' && (
                          <span className={classes.cellCount}>
                            {cell.deviationCount} event{cell.deviationCount !== 1 ? 's' : ''}
                          </span>
                        )}
                      </div>
                    </Tooltip>
        ...
        {/* ── Legend ── */}
        <Box display="flex" alignItems="center" style={{ gap: 16, flexWrap: 'wrap' }}>
          <Typography style={{ fontSize: '0.65rem', color: '#94A3B8', fontFamily: 'monospace', fontWeight: 700, textTransform: 'uppercase' }}>
            Legend:
          </Typography>
          {(['none', 'low', 'medium', 'high', 'critical'] as const).map(sev => (
            <span key={sev}>
              <span
                ...
                style={{ background: SEVERITY_PALETTE[sev].bg, border: `1px solid ${SEVERITY_PALETTE[sev].border}` }}
              />
          ...
          <Typography style={{ fontSize: '0.62rem', color: '#CBD5E1', fontFamily: 'monospace', marginLeft: 'auto' }}>
            Rows = workflows · Columns = event types · Click any cell for details
          </Typography>
        </Box>
