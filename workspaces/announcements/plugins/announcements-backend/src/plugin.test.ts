...
import { announcementsPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    ...
    await request(server).get('/api/announcements/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/announcements/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/announcements/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/announcements/todos/${createdTodoItem.id}`)
