...
export class TagsDatabase {
  ...
  async delete(slug: string): Promise<void> {
    try {
      const isPostgres = this.db.client.config.client === 'pg';
      ...
      if (isPostgres) {
        ...
        referencedCount = parseInt(result.rows[0].count, 10);
      } else {
        const result = await this.db.raw(
          `
        ...
        referencedCount = result[0].count;
      }
      ...
      // Do not throw error if tag is not found
    } catch (error) {
      throw error;
    }
  }
