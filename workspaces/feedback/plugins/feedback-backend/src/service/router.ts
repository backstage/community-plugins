...
import { feedbackResourceRef, isFeedbackOwner } from '../permissions';
...
export async function createRouter(
  ...
  permissionsRegistry.addResourceType({
    ...
    permissions: [feedbackUpdatePermission, feedbackDeletePermission],
    getResources: async feedbackIds => {
      const feedbacks: FeedbackModel[] = [];
      for (const feedbackId of feedbackIds) {
        const feedback = await feedbackDB.getFeedbackByUuid(feedbackId);
  ...
  router.post('/', async (req, res) => {
    const [decision] = await permissions.authorize(
      [{ permission: feedbackCreatePermission }],
    ...
    const reqCredentials = (await httpAuth.credentials(
      req,
    )) as BackstageCredentials<
      BackstageUserPrincipal | BackstageServicePrincipal
    >;
    reqData.createdBy =
      reqCredentials.principal.type === 'user'
        ? reqCredentials.principal.userEntityRef
        : reqCredentials.principal.subject;
        ...
        mailer.sendMail({
          ...
          body: `
            <div>
              ...
              ${
                reqData.ticketUrl
                  ? `Jira Ticket: <a href="${reqData.ticketUrl}">${reqData.ticketUrl}</a>
              <br/>
              <br/>`
                  : ''
              }
  ...
  router.get('/', async (req, res) => {
    const [decision] = await permissions.authorize(
      [{ permission: feedbackReadPermission }],
      { credentials: await httpAuth.credentials(req) },
    );
    if (decision.result === AuthorizeResult.DENY) {
      throw new NotAllowedError('Unauthorized');
  ...
  router.get('/:id/ticket', async (req, res) => {
    const ticketId = req.query.ticketId ? (req.query.ticketId as string) : null;
    const projectId = req.query.projectId
      ? (req.query.projectId as string)
      : null;
  ...
  router.patch('/:id', async (req, res) => {
    ...
    const [decision] = await permissions.authorize(
      [{ permission: feedbackUpdatePermission, resourceRef: feedbackId }],
      { credentials: await httpAuth.credentials(req) },
    );
    if (decision.result === AuthorizeResult.DENY) {
      throw new NotAllowedError('Unauthorized');
  ...
  router.delete('/:id', async (req, res) => {
    ...
    const [decision] = await permissions.authorize(
      [{ permission: feedbackDeletePermission, resourceRef: feedbackId }],
      { credentials: await httpAuth.credentials(req) },
    );
    if (decision.result === AuthorizeResult.DENY) {
      throw new NotAllowedError('Unauthorized');
    ...
    if (await feedbackDB.checkFeedbackId(feedbackId)) {
      ...
      return res
        .status(200)
        .json({ feedbackId, message: 'Deleted successfully' });
    }
