...
jest.mock('../../hooks/useDarkTheme', () => ({
  useDarkTheme: jest.fn(),
}));
...
const useDarkThemeMock = useDarkTheme as jest.Mock;
...
describe('TopologyComponent', () => {
  beforeEach(() => {
    useDarkThemeMock.mockImplementation(() => {});
  ...
  it('should invoke useDarkTheme', () => {
    mockUsePermission.mockReturnValue({ loading: false, allowed: true });
    render(<TopologyComponent />);
    expect(useDarkThemeMock).toHaveBeenCalled();
  });
});
