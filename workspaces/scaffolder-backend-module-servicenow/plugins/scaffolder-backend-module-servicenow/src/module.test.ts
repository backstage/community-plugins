...
describe('scaffolderModuleServicenowActions', () => {
  it('registers all six ServiceNow table actions via the scaffolder extension point', async () => {
    ...
    await startTestBackend({
      ...
      features: [
        scaffolderModuleServicenowActions,
    ...
    expect(registeredIds).toEqual([...EXPECTED_ACTION_IDS]);
  });

  it('still registers all six actions when servicenow config is absent', async () => {
    const registeredIds: string[] = [];
    const extensionPoint = {
      addActions: (...actions: { id: string }[]) => {
        registeredIds.push(...actions.map(action => action.id));
