...
import { backstageMcpPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [backstageMcpPlugin],
    });

    await request(server).get('/api/backstage-mcp-plugin/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/backstage-mcp-plugin/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/backstage-mcp-plugin/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/backstage-mcp-plugin/todos/${createdTodoItem.id}`)
