...
import { Application, Source } from '@backstage-community/plugin-argocd-common';
...
describe('DeploymentLifecycleDrawer', () => {
  ...
  test('should not render the commit section for helm based applications in drawer component', () => {
    const helmApplication = {
      ...
      spec: {
        ...
        source: { ...mockApplication.spec.source, chart: 'some-charts' },
      },
    };
    (useDrawerContext as jest.Mock).mockReturnValue({
  ...
  test('should hide cluster when showServer prop is false', () => {
    render(
      <DeploymentLifecycleDrawer
        isOpen
        onClose={jest.fn()}
  ...
  test('should hide both instance and cluster when both props are false', () => {
    render(
      <DeploymentLifecycleDrawer
        isOpen
        onClose={jest.fn()}
