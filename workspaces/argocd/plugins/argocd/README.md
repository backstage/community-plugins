...
> Tip: You can use the [prepared manifest for a read-only ClusterRole](https://raw.githubusercontent.com/backstage/community-plugins/main/workspaces/argocd/plugins/argocd/manifests/clusterrole.yaml), which provides access for both Kubernetes plugin and Argo CD plugin.
...
The Argo CD entity tabs also use an extension `if` predicate so they are only shown when the signed-in user is authorized for `argocd.view.read`. Users without that permission will not see the **Deployment Lifecycle** or **Deployment Summary** tabs on the entity page.
