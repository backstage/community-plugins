...
The multi-user config overlays read secrets from environment variables. Set the following before running any multi-user command:
...
#### Browser-based testing (frontend + backend)

After setting [environment variables](#environment-setup) and [starting Keycloak](#starting-keycloak), run both frontend and backend with a single command:
...
#### CLI-only testing (backend only)

After setting [environment variables](#environment-setup) and [starting Keycloak](#starting-keycloak), start the backend:
