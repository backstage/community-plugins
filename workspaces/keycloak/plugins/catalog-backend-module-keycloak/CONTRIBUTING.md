...
| `401 Unauthorized` on catalog `curl`                                       | Harness loaded workspace root config (no `backend.auth.externalAccess`) or stale static token                                                                                               | Restart harness after `eval "$(./scripts/export-dev-env-from-realm.sh)"`; confirm startup log loads **plugin** `app-config.yaml`, not only workspace root; stop other backends on **7007**                                                                                                                                                                                           |
| `401` / `Illegal token` on catalog `curl`                                  | Wrong backend on the port, harness started before `eval`, missing Bearer header, or token mismatch after env refresh                                                                        | Run `eval` first, restart `yarn workspace @backstage-community/plugin-catalog-backend-module-keycloak start`, confirm logs show this package's `app-config.yaml` and the expected listen port; if the error stack mentions another workspace (for example `workspaces/rbac`), stop that backend or use the port from the Keycloak harness log                                        |
...
| `Cannot refresh token: missing refresh token or credentials` on first sync | Docker Desktop VM clock drifted from the host (common on macOS after Docker slept) — Keycloak JWTs look expired immediately and the admin client cannot refresh `client_credentials` tokens | Compare clocks: `date -u` and `docker exec $(docker ps -q --filter ancestor=quay.io/keycloak/keycloak:22.0.1) date -u`. If they differ, **restarting the Keycloak container alone does not fix this** — restart Docker Desktop, then start Keycloak again with `yarn workspace @backstage-community/plugin-catalog-backend-module-keycloak start:keycloak`, and restart this harness |
