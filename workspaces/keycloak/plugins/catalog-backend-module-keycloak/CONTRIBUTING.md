...
| `Cannot refresh token: missing refresh token or credentials` on first sync | Podman VM clock drifted from the host (common on macOS after the Podman machine slept) — Keycloak JWTs look expired immediately and the admin client cannot refresh `client_credentials` tokens | Compare clocks: `date -u` and `podman exec $(podman ps -q --filter 'ancestor=quay.io/keycloak/keycloak:22.0.1') date -u`. If they differ, **restarting the Keycloak container alone does not fix this** — on macOS run `podman machine stop && podman machine start`, then start Keycloak again with `yarn workspace @backstage-community/plugin-catalog-backend-module-keycloak start:keycloak`, and restart this harness |
