...
import { Application, Identity, mtaApiRef, Target, TaskDashboard } from '../api/api';
...
export const useAnalyzeApplication = (options?: UseAnalyzeApplicationOptions) => {
  const api = useApi(mtaApiRef);
  const queryClient = useQueryClient();
...
export const useFetchAppTasks = (id: number) => {
  const api = useApi(mtaApiRef);
  const { error, data, isError, isFetching, refetch } = useQuery<TaskDashboard[]>({
