...
export const ApplicationDetailsForm = ({
  ...
  const getDefaultIdentityValue = (appIdentities: Ref[], kind: string) => {
    const appIdentityRef = appIdentities?.find(appIdentity =>
      identities?.some(identity => identity.id === appIdentity.id && identity.kind === kind),
    );
  ...
  const onSubmit = (formData: any, event: any) => {
    ...
    const mavenIdentity =
      formData.mavenCredentials !== 'None' ? findIdentityByName(formData.mavenCredentials) : null;

    const sourceIdentity =
      formData.sourceCredentials !== 'None' ? findIdentityByName(formData.sourceCredentials) : null;
    ...
    return (
      <Paper style={{ padding: '16px', backgroundColor: '#fff3f3' }}>
        ...
        <Typography variant="body1">
          Unable to update application details. This is likely because your Backstage client in
          Keycloak is missing the required scopes.
        </Typography>
        <Typography variant="body2" style={{ marginTop: '8px' }}>
          Please ensure a Backstage client is added to your Keycloak realm with the necessary scopes
          (applications:get, applications:put, identities:get, etc.) to make requests against MTA.
        </Typography>
        <Typography variant="body2" style={{ marginTop: '8px' }}>
          See{' '}
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Grid item xs={12}>
          <FormControl fullWidth error={!!errors.sourceCredentials}>
            ...
            {errors.sourceCredentials && (
              <Typography color="error">{errors.sourceCredentials.message}</Typography>
            )}
          </FormControl>
        </Grid>
        <Grid item xs={12}>
          <FormControl fullWidth error={!!errors.mavenCredentials}>
            ...
            {errors.mavenCredentials && (
              <Typography color="error">{errors.mavenCredentials.message}</Typography>
            )}
          </FormControl>
        </Grid>
        <Grid item xs={12}>
      ...
      <Snackbar open={showErrorSnackbar} autoHideDuration={6000} onClose={handleCloseSnackbar}>
        <SnackbarContent
          ...
          message={
            <span>
              Authentication error (401). Your Backstage client in Keycloak may be missing the
              required scopes (applications:get, applications:put, identities:get, etc.).
            </span>
          }
          action={
