/**
 * Type override for jsep@1.3.9.
 *
 * The upstream node_modules/jsep/typings/tsd.d.ts uses `export = jsep`
 * which is incompatible with TypeScript 5.8+ when compiled with
 * `--skipLibCheck false` and `module: ES2020` (error TS1203).
 * This file replaces `export =` with `export default` to fix the issue.
 *
 * This error only surfaces via the `yarn tsc:full` script, which
 * explicitly passes `--skipLibCheck false`. Normal `tsc` is unaffected.
 *
 * Referenced by `compilerOptions.paths` in tsconfig.json.
 *
 * To check if this override is still needed:
 *   npx tsc --skipLibCheck false --incremental false 2>&1 | grep TS1203
 * If no TS1203 errors appear, remove this file and the paths entry.
 *
 * Update this file when jsep is upgraded by diffing against
 * node_modules/jsep/typings/tsd.d.ts.
 */
declare module 'jsep' {
  namespace jsep {
