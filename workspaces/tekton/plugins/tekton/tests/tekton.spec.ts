...
test.describe('Tekton plugin', () => {
  ...
  test.describe('Missing permissions page', () => {
    test('handles missing kubernetes read permissions', async ({}, testInfo) => {
      ...
      await expect(
        page.getByText(translations.permissionAlert.title, { exact: true }),
      ).toBeVisible();
  ...
  test.describe('Tekton view', () => {
    ...
    test.describe('Pipeline with scanner', () => {
      ...
      test('Output action is available', async () => {
        ...
        await btn.click();
        await expect(
          page.getByTestId('pipelinerun-output-dialog'),
        ).toBeVisible();
      });
      ...
      test('Enterprise contract output is shown', async () => {
        ...
        await expect(card).toContainText(
          'Enterprise Contract is a set of tools',
        );
    ...
    test.describe('Pipeline with external sbom', () => {
      ...
      test('Show sbom action points to quay.io', async () => {
        ...
        expect(await showSbom.getAttribute('href')).toContain(
          'https://quay.io',
        );
      });
