...
import { SonarCloudApiError, SonarCloudTimeoutError } from './errors';
...
describe('SonarCloudClient', () => {
  ...
  describe('baseUrl', () => {
    ...
    it('should strip trailing slash from baseUrl', async () => {
      ...
      const url = fetchSpy.mock.calls[0][0] as string;
      expect(url).not.toContain('custom.example.com//');
  ...
  describe('retry on 429', () => {
    ...
    it('should re-throw original 429 error when all retries exhausted', async () => {
      fetchSpy
        .mockResolvedValueOnce(new Response(null, { status: 429 }))
        .mockResolvedValueOnce(new Response(null, { status: 429 }))
        .mockResolvedValueOnce(new Response(null, { status: 429 }))
      ...
      await promise;
      expect(caughtError).toBeInstanceOf(SonarCloudApiError);
      expect((caughtError as SonarCloudApiError).status).toBe(429);
    });
    ...
    it('should re-throw 429 when retry sequence cap would be exceeded', async () => {
      let callCount = 0;
