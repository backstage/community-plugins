  ...
  static fromConfig(config: Config): SonarqubeConfig {
    ...
    const namedInstanceConfig =
      sonarqubeConfig.getOptionalConfigArray('instances')?.map(c => ({
        ...
        apiKeyKind:
          (c.getOptionalString('apiKeyKind') as 'Bearer' | 'Basic') ?? 'Basic',
      })) || [];
    ...
    const apiKeyKind =
      (sonarqubeConfig.getOptionalString('apiKeyKind') as 'Bearer' | 'Basic') ??
      'Basic';
...
export class DefaultSonarqubeInfoProvider implements SonarqubeInfoProvider {
  ...
  private async getSupportedMetrics(
    ...
    for (;;) {
      const result = await this.callApi<{
        ...
        total: number;
      }>(instanceUrl, 'api/metrics/search', token, apiKeyKind, {
        ps: 500,
        p: nextPage,
      });
  ...
  private async callApi<T>(
    ...
    const response = await fetch(fullUrl, {
      headers: {
        ...
        Authorization: `${apiKeyKind} ${encodedAuthToken}`,
      },
    });
    if (!response.ok) {
  ...
  async getFindings(options: {
    ...
    const { baseUrl, apiKey, apiKeyKind } = this.config.getInstanceConfig({
