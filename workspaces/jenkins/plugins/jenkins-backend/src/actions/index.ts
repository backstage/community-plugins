...
export const createJenkinsActions = (options: {
  actionsRegistry: ActionsRegistryService;
  jenkinsService: JenkinsService;
}) => {
  createGetProjectsAction(options);
  createGetBuildAction(options);
  createGetJobBuildsAction(options);
  createRebuildProjectAction(options);
  createGetBuildConsoleTextAction(options);
};
