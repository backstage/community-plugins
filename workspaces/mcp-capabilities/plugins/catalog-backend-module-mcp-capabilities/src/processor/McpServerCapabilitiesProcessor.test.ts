...
} from './McpServerCapabilitiesProcessor';
...
function installMockClient(overrides: Record<string, jest.Mock> = {}) {
  const client = {
    connect: jest.fn().mockResolvedValue(undefined),
    getServerCapabilities: jest.fn().mockReturnValue({ tools: {} }),
    getServerVersion: jest
      .fn()
      .mockReturnValue({ name: 'demo', version: '1.0.0' }),
...
const processor = (fields?: EnrichmentFields) =>
  new McpServerCapabilitiesProcessor({ logger, fields });
...
describe('McpServerCapabilitiesProcessor', () => {
  ...
  it('leaves non-mcp-server entities untouched, never connecting', async () => {
    const entity: Entity = {
      apiVersion: 'backstage.io/v1alpha1',
      kind: 'API',
  ...
  it('persists only names + counts when fields is "names"', async () => {
    const out = await processor('names').preProcessEntity(
      mcpEntity,
      anyLocation,
      emit,
      anyLocation,
    ...
    expect(out.spec).not.toHaveProperty('capabilities');
    expect(out.spec).not.toHaveProperty('serverInfo');
  });
