# Contributing — Annotator scaffolder backend module

Developer guide for `@backstage-community/plugin-scaffolder-backend-module-annotator`. For operator install and configuration, see [README.md](./README.md).
...
From the workspace root (`workspaces/scaffolder-backend-module-annotator`):

```bash
yarn workspace @backstage-community/plugin-scaffolder-backend-module-annotator test
yarn workspace @backstage-community/plugin-scaffolder-backend-module-annotator lint:check
yarn tsc
```
...
- **Module wiring** — scaffolder extension-point registration and all four action IDs
- **Resolver utilities** — `resolveAnnotation`, `resolveSpec`, `getObjectToAnnotate`, and timestamp formatting contracts
- **Action handlers** — annotation merge behavior and disk-write results for representative action cases
...
2. List registered scaffolder actions (requires Bearer token):
   ...
   Expect all four action IDs in the response:
