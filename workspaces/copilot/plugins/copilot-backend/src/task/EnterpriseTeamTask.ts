...
export async function discoverEnterpriseTeamMetrics({ api, logger, db, config }: TaskOptions): Promise<void> {
  if (!config.getOptionalString('copilot.enterprise')) {
    logger.info('[discoverEnterpriseTeamMetrics] Skipping: Enterprise configuration not found.');
    return;
  }
  ...
  try {
    ...
    logger.info(`[discoverEnterpriseTeamMetrics] Fetched ${teams.length} teams`);

    for (const team of teams) {
      try {
        logger.info(`[discoverEnterpriseTeamMetrics] Fetching metrics for team: ${team.slug}`);

        const copilotMetrics = await api.fetchEnterpriseTeamCopilotMetrics(team.slug);

        const lastDay = await db.getMostRecentDayFromMetricsV2(type, team.slug);
        logger.info(`[discoverEnterpriseTeamMetrics] Found last day: ${lastDay}`);
        ...
        if (newMetrics.length > 0) {
          ...
          logger.info(`[discoverEnterpriseTeamMetrics] Inserted new metrics into the database for team: ${team.slug}`);
        } else {
          logger.info(`[discoverEnterpriseTeamMetrics] No new metrics found to insert for team: ${team.slug}`);
        }
      } catch (error) {
        logger.error(`[discoverEnterpriseTeamMetrics] Error processing metrics for team ${team.slug}: ${error}`);
      }
    }
  } catch (error) {
