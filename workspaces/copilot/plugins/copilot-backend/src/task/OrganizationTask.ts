 ...
 */
import { MetricsType, CopilotMetrics } from '@backstage-community/plugin-copilot-common';
...
export async function discoverOrganizationMetrics({ api, logger, db, config }: TaskOptions): Promise<void> {
  if (!config.getOptionalString('copilot.organization')) {
    logger.info('[discoverOrganizationMetrics] Skipping: Organization configuration not found.');
    return;
  }
  ...
  try {
    ...
    logger.info(`[discoverOrganizationMetrics] Fetched ${copilotMetrics.length} metrics`);
    ...
    logger.info(`[discoverOrganizationMetrics] Found ${newMetrics.length} new metrics to insert`);

    if (newMetrics.length > 0) {
      const coPilotMetrics = filterBaseMetrics(newMetrics, type);
      const ideCompletionsToInsert = filterIdeCompletionMetrics(newMetrics, type);
      const ideCompletionsLanguagesToInsert = filterIdeCompletionLanguageMetrics(newMetrics, type);
      const ideCompletionsEditorsToInsert = filterIdeCompletionEditorMetrics(newMetrics, type);
      const ideCompletionsEditorModelsToInsert = filterIdeCompletionEditorModelMetrics(newMetrics, type);
      ...
      const ideChatEditorModels = filterIdeChatEditorModelMetrics(newMetrics, type);
      ...
      logger.info('[discoverOrganizationMetrics] Inserted new metrics into the database');
    } else {
      logger.info('[discoverOrganizationMetrics] No new metrics found to insert');
    }
  } catch (error) {
    logger.error(`[discoverOrganizationMetrics] An error occurred while processing Github Copilot metrics: ${error}`);
    throw error;
  }
}
