...
export async function discoverEnterpriseMetrics({ api, logger, db, config }: TaskOptions): Promise<void> {
  if (!config.getOptionalString('copilot.enterprise')) {
    logger.info('[discoverEnterpriseMetrics] Skipping: Enterprise configuration not found.');
    return;
  }
  ...
  try {
    ...
    logger.info(`[discoverEnterpriseMetrics] Fetched ${copilotMetrics.length} metrics`);
    ...
    logger.info(`[discoverEnterpriseMetrics] Found ${newMetrics.length} new metrics to insert`);
    ...
    if (newMetrics.length > 0) {
      ...
      logger.info('[discoverEnterpriseMetrics] Inserted new metrics into the database');
    } else {
      logger.info('[discoverEnterpriseMetrics] No new metrics found to insert');
    }
  } catch (error) {
    logger.error(`[discoverEnterpriseMetrics] An error occurred while processing Github Copilot metrics: ${error}`);
    throw error;
  }
}
