...
export class OrganizationTeamProcessor implements IProcessor {
  name: string;

  constructor(private readonly options: Options) {
    this.name = 'OrganizationTeamProcessor';
  }
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const teams = await this.options.api.fetchOrganizationTeams();
      ...
      for (const team of teams) {
        try {
          ...
          const metrics =
            await this.options.api.fetchOrganizationTeamCopilotUsage(team.slug);
