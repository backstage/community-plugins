...
export class ServiceHealth extends Health {
  public static fromJson = (
    ...
    ctx: HealthContext,
    serverConfig: ComputedServerConfig,
  ) => new ServiceHealth(ns, srv, json.requests, ctx, serverConfig);
    ...
    if (ctx.hasSidecar) {
      ...
      const item: HealthItem = {
        title: createTrafficTitle(
          getName(ctx.rateInterval, serverConfig).toLocaleLowerCase('en-US'),
        ),
  ...
  constructor(
    ...
    super(ServiceHealth.computeItems(ns, srv, requests, ctx, serverConfig));
  }
}
