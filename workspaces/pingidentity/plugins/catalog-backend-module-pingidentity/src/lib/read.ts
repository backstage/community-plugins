...
const parsePingIdentityUsers = async (
  ...
  const transformedUsers: (UserEntity | undefined)[] = await Promise.all(
    pingIdentityUsers.map(async (user: any) => {
...
const parsePingIdentityGroups = async (
  ...
  const transformedGroups: (GroupEntity | undefined)[] = await Promise.all(
    pingIdentityGroups.map(async (group: any) => {
...
export const readPingIdentity = async (
  ...
  const groups: GroupEntity[] = await parsePingIdentityGroups(
    client,
    groupMembersMap,
    parentGroupMap,
  ...
  const users: UserEntity[] = await parsePingIdentityUsers(
    client,
    groups,
    groupMembersMap,
