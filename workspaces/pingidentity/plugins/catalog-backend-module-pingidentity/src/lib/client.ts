...
class PingIdentityClient {
   ...
   */
  async getUsers(
    querySize: number = PING_IDENTITY_DEFAULT_ENTITY_QUERY_SIZE,
  ): Promise<PingIdentityUser[]> {
    ...
    while (nextUrl) {
      const url = nextUrl.startsWith('http')
        ? nextUrl
        : `${this.config.apiPath}/environments/${this.config.envId}/${nextUrl}`;
      const response = await this.requestApi(url, true);
      const data: PingIdentityResponse =
        (await response.json()) as PingIdentityResponse;
  ...
  async getUsersInGroup(groupId: string): Promise<string[]> {
    ...
    const data = await response.json();
    return data.count > 0
      ? data._embedded.users.map((users: { id: string }) => users.id)
      : [];
  }
  ...
  async requestApi(query: string, isFullUrl?: boolean): Promise<Response> {
    const url = isFullUrl
      ? query
      : `${this.config.apiPath}/environments/${this.config.envId}/${query}`;
