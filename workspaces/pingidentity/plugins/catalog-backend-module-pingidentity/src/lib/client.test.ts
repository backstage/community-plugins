...
describe('PingIdentityClient', () => {
  ...
  it('should fetch access token and make API request', async () => {
    worker.use(
      ...
      rest.get(
        'https://api.example.com/environments/envId/query',
        (_, res, ctx) => {
  ...
  it('should fetch users', async () => {
    worker.use(
      ...
      rest.get(
        'https://api.example.com/environments/envId/users',
        (_, res, ctx) => {
          return res(
            ctx.status(200),
            ctx.json({
  ...
  it('should fetch groups', async () => {
    worker.use(
      ...
      rest.get(
        'https://api.example.com/environments/envId/groups',
        (_, res, ctx) => {
          return res(
            ctx.status(200),
            ctx.json({
  ...
  it('should fetch parent group', async () => {
    worker.use(
      ...
      rest.get(
        'https://api.example.com/environments/envId/groups/group1/memberOfGroups',
        (_, res, ctx) => {
          return res(
            ctx.status(200),
            ctx.json({
              size: 1,
              _embedded: {
                groupMemberships: [{ id: 'ParentGroup' }],
              },
            }),
          );
        },
  ...
  it('should fetch users in a group', async () => {
    worker.use(
      ...
      rest.get(
        'https://api.example.com/environments/envId/users',
        (req, res, ctx) => {
          expect(req.url.searchParams.get('filter')).toBe(
            'memberOfGroups[id eq "group1"]',
          );
          return res(
            ctx.status(200),
            ctx.json({
