...
import { pingfederateAuthenticator } from './authenticator';
...
describe('pingfederateAuthenticator', () => {
  ...
  beforeEach(() => {
    ...
    implementation = pingfederateAuthenticator.initialize({
  ...
  describe('#start', () => {
    ...
    it('redirects to authorization endpoint returned from OIDC metadata endpoint', async () => {
      const startResponse = await pingfederateAuthenticator.start(
        startRequest,
        implementation,
      );
    ...
    it('stores PKCE verifier in session', async () => {
      await pingfederateAuthenticator.start(startRequest, implementation);
    ...
    it('fails when request has no session', async () => {
      return expect(
        pingfederateAuthenticator.start(
          {
            state: encodeOAuthState(oauthState),
            req: {
              method: 'GET',
  ...
  describe('#authenticate', () => {
    ...
    it('exchanges authorization code for access token', async () => {
      const authenticatorResult = await pingfederateAuthenticator.authenticate(
        handlerRequest,
        implementation,
      );
    ...
    it('fails without authorization code', async () => {
      ...
      return expect(
        pingfederateAuthenticator.authenticate(handlerRequest, implementation),
      ).rejects.toThrow('Unexpected redirect');
    });

    it('fails without oauth state', async () => {
      return expect(
        pingfederateAuthenticator.authenticate(
          {
            req: {
              method: 'GET',
  ...
  describe('#refresh', () => {
    ...
    it('gets new refresh token', async () => {
      const refreshResponse = await pingfederateAuthenticator.refresh(
        refreshRequest,
        implementation,
      );
  ...
  describe('#logout', () => {
    it('should revoke refreshToken', async () => {
      ...
      await pingfederateAuthenticator.logout?.(logoutRequest, implementation);

      const refreshResponse = pingfederateAuthenticator.refresh(
        refreshRequest,
        implementation,
      );
    ...
    it('should not revoke refreshToken when issuer revocation_endpoint is undefined', async () => {
      ...
      const newImplementation = pingfederateAuthenticator.initialize({
        callbackUrl: 'https://backstage.test/callback',
        config: new ConfigReader({
          baseUrl: 'https://oidc.test',
          clientId: 'clientId',
      ...
      await pingfederateAuthenticator.logout?.(
        logoutRequest,
        newImplementation,
      );
