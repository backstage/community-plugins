...
import { authModulePingFederateProvider } from './module';
import authPlugin from '@backstage/plugin-auth-backend';

describe('authModulePingFederateProvider', () => {
  ...
  beforeEach(async () => {
    ...
    const backend = await startTestBackend({
      features: [
        authModulePingFederateProvider,
        authPlugin,
  ...
  it('should start', async () => {
    ...
    const nonceCookie = agent.jar.getCookie('pingfederate-nonce', {
      domain: '127.0.0.1',
      path: '/api/auth/pingfederate/handler',
      script: false,
      secure: false,
    });
    ...
    expect(expected).toEqual({
      ...
      client_id: 'clientId',
      redirect_uri: `${appUrl}/api/auth/pingfederate/handler/frame`,
