...
export const pointingPokerPlugin = createBackendPlugin({
  ...
  register(env) {
    ...
    env.registerInit({
      ...
      async init({ catalog, config, httpAuth, httpRouter, logger, database }) {
        const db = await DatabaseClient.create(database, {
          staleSessionHours: config.getOptionalNumber(
            'pointingPoker.staleSessionHours',
          ),
          storySortGap: config.getOptionalNumber('pointingPoker.storySortGap'),
        });
