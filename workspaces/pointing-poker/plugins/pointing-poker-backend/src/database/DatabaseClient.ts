...
const DEFAULT_STALE_SESSION_HOURS = 8;
...
export class DatabaseClient {
  private constructor(
    ...
    private readonly staleSessionHours: number,
    private readonly sortGap: number,
  ) {}

  static async create(
    database: DatabaseService,
    options?: { staleSessionHours?: number; storySortGap?: number },
  ): Promise<DatabaseClient> {
    const staleSessionHours =
      options?.staleSessionHours ?? DEFAULT_STALE_SESSION_HOURS;
    const sortGap = options?.storySortGap ?? DEFAULT_SORT_GAP;
    if (!(staleSessionHours > 0)) {
      throw new InputError('pointingPoker.staleSessionHours must be positive');
    }
    if (!(sortGap > 0)) {
      throw new InputError('pointingPoker.storySortGap must be positive');
    }
    ...
    return new DatabaseClient(client, staleSessionHours, sortGap);
  }
  ...
  /** Roles the user holds in sessions that contain the given ticket. */
  async getTicketRoles(
    userId: string,
    ticketKey: string,
  ): Promise<ParticipantRole[]> {
    const rows = await this.db('pointing_poker_participants as p')
      .join('pointing_poker_stories as s', 's.session_id', 'p.session_id')
      .where('p.user_id', userId)
      .andWhere('s.ticket_key', ticketKey)
      .distinct('p.role');
    return rows.map(row => row.role as ParticipantRole);
  }
  ...
  async splitStory(
    ...
    const gapEnd =
      nextStory?.sort ??
      parent.sort + this.sortGap * subtasks.length + this.sortGap;
  ...
  private async autoCompleteStaleSessions(): Promise<void> {
    const cutoff = new Date(
      Date.now() - this.staleSessionHours * 60 * 60 * 1000,
    );
