...
const createChip = (user: MentionUser): HTMLSpanElement => {
  ...
  chip.style.color = 'var(--bui-fg-announcement)';
...
>(({ onChange, placeholder = 'Add a comment…' }, ref) => {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <style>{`.${EDITOR_CLASS}:empty:before{content:attr(data-placeholder);color:var(--bui-fg-secondary);}.${EDITOR_CLASS}:focus-visible{outline:2px solid var(--bui-announcement-border);outline-offset:-1px;}`}</style>
      {showDropdown && (
        <div
          style={{
