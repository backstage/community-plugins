...
| `@backstage-community/plugin-pointing-poker-backend-module-jira` | Jira search, issue context, comments, and story-point write-back     |
...
yarn --cwd packages/backend add \
  ...
  @backstage-community/plugin-pointing-poker-backend-module-jira
```
...
`yarn start` runs the plugin dev apps: the frontend at `http://localhost:3000` (with a mocked API and sample catalog data) and the backend at `http://localhost:7007` with an in-memory SQLite database. To try the Jira module against a real site, export `JIRA_HOST`, `JIRA_USER_EMAIL` and `JIRA_API_TOKEN` before starting.
