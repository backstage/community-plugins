...
describe('RollbarApi', () => {
  ...
  describe('getProject', () => {
    ...
    it('can use project map class instance state', async () => {
      setupHandlers();
      const api = new RollbarApi('my-access-token', mockServices.rootLogger());
      const spy = jest.spyOn(api as any, 'getAllProjects');
      const project1 = await api.getProject(mockProject.name);
      const project2 = await api.getProject(mockProject.name);
      expect(project1.name).toEqual(mockProject.name);
      expect(project2.name).toEqual(mockProject.name);
